	...
	m_engine(engine), m_title(tile), m_bShow(true), m_bLastShow(true)
{
	...
	// widget id plus one to ensure always get unique id.
	s_widgetId ++;
}
...
void Widget::init()
{
	...
	m_uiSystem->addImguiFunction(getUniqueId(), [this](const TickData& tickData, size_t i)
	{
...
void Widget::release()
{
	...
	m_uiSystem->removeImguiFunction(getUniqueId());
