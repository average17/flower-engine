...
namespace Flower
{
	AccelerateStructure::~AccelerateStructure()
	{
		...
		{
			vkDestroyAccelerationStructureKHR(RHI::Device, m_handle, nullptr);
		}
	}
	...
	void AccelerateStructure::buildAndFlushOnGraphicsQueue(VkBuildAccelerationStructureFlagsKHR flags, VkBuildAccelerationStructureModeKHR mode)
	{
		...
		if (!m_buffer || m_buffer->getSize() != m_size.accelerationStructureSize)
		{
			...
			RHICheck(vkCreateAccelerationStructureKHR(RHI::Device, &acceleration_structure_create_info, nullptr, &m_handle));
		}
		...
		m_deviceAddress = vkGetAccelerationStructureDeviceAddressKHR(RHI::Device, &acceleration_device_address_info);

		// Create a scratch buffer as a temporary storage for the acceleration structure build
		auto scratchBuffer = VulkanBuffer::createRTScratchBuffer("ScatchBuffer", m_size.buildScratchSize); 

		build_geometry_info.scratchData.deviceAddress = scratchBuffer->getDeviceAddress();
		build_geometry_info.dstAccelerationStructure = m_handle;
