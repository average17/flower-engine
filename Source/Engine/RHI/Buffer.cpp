...
namespace flower
{
	static AutoCVarInt32 cVarEnableVma(
		...
		"RHI",
		1, // when vram > 256 MB, may allocate fail on some machine.
		...
		bool bHeapMemory)
	{
		...
		ret_buffer->m_usageFlags = usageFlags;
		ret_buffer->m_memoryPropertyFlags = memoryPropertyFlags;
		ret_buffer->m_isHeap = bHeapMemory;
	...
	VkResult VulkanBuffer::map(VkDeviceSize size, VkDeviceSize offset)
	{
		...
		if(!isHeap())
		{
			res = vmaMapMemory(RHI::get()->getVmaAllocator(),m_allocation,&mapped);
		}
		else
		{
	...
	void VulkanBuffer::unmap()
	{
		if (mapped)
		{
			if(!isHeap())
			{
				vmaUnmapMemory(RHI::get()->getVmaAllocator(),m_allocation);
				mapped = nullptr;
			}
			else
			{
	...
	VkResult VulkanBuffer::bind(VkDeviceSize offset)
	{
		if(!isHeap())
		{
			return vmaBindBufferMemory2(RHI::get()->getVmaAllocator(), m_allocation,offset,m_buffer,nullptr);
		}
		else
		{
	...
	VkResult VulkanBuffer::flush(VkDeviceSize size, VkDeviceSize offset)
	{
		...
		if(!isHeap())
		{
			res = vmaFlushAllocation(RHI::get()->getVmaAllocator(),m_allocation,offset,size);
		}
		else
		{
	...
	VkResult VulkanBuffer::invalidate(VkDeviceSize size, VkDeviceSize offset)
	{
		...
		if(!isHeap())
		{
			res = vmaInvalidateAllocation(RHI::get()->getVmaAllocator(),m_allocation,offset,size);
		}
		else
		{
			VkMappedMemoryRange mappedRange = {};
		...
		bool bHeapMemory) 
	{
		...
		if(!isHeap())
		{
			...
			vkCheck(vmaCreateBuffer(RHI::get()->getVmaAllocator(), &bufferInfo, &vmaallocInfo,
				&m_buffer,
				&m_allocation,
				nullptr));
			...
			{
				void* mapped;
				vmaMapMemory(RHI::get()->getVmaAllocator(), m_allocation, &mapped);
				memcpy(mapped, data, size);
				vmaUnmapMemory(RHI::get()->getVmaAllocator(), m_allocation);
			}

			// vma already bind buffer memory.
			// vmaBindBufferMemory(RHI::get()->GetVmaAllocator(),m_allocation,buffer);
		}
		else
		{
	...
	void VulkanBuffer::destroy()
	{
		if(!isHeap())
		{
			vmaDestroyBuffer(RHI::get()->getVmaAllocator(), m_buffer, m_allocation);
		}
		else
		{
		...
		VkIndexType indexType)
	{
		...
		ret->m_indexType = indexType;

		if(indexType==VK_INDEX_TYPE_UINT16)
		{
			ret->m_indexCount = (uint32_t)bufferSize / (uint32_t)sizeof(uint16_t);
		}
		else if(indexType==VK_INDEX_TYPE_UINT32)
		{
			ret->m_indexCount = (uint32_t)bufferSize / (uint32_t)sizeof(uint32_t);
		}
		else
		{
			LOG_FATAL("No support index type!");
		}
